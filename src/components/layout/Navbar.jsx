...
export default function Navbar({ location, toggleTheme, theme, setCurrentView, userInitials = "B", activeFilter, setActiveFilter, onLogout, onViewProfile, onBackToOneHome }) {
    ...
    return (
        <nav className={`w-full px-4 lg:px-8 py-3 lg:py-4 flex items-center justify-between gap-3 lg:gap-6 z-[120] shrink-0 sticky top-0 backdrop-blur-xl border-b min-h-[56px] lg:min-h-16 transition-all duration-300 ${colors.isDark ? 'bg-warm-950/85 border-white/10' : 'bg-white/85 border-stone-200/80'} shadow-[0_4px_20px_rgba(180,160,140,0.08)]`}>
            ...
            >
                <div className="relative w-8 h-8 rounded-full bg-[#0f172a] shadow-sm shrink-0 flex items-center justify-center group-hover:scale-105 transition-transform">
                    <div className="w-full h-full rounded-full flex items-center justify-center text-xs font-bold text-white">
                        HS
                    </div>
                </div>
                <div className="flex flex-col min-w-0">
                    <h2 className={`text-base font-bold tracking-tight ${colors.text}`}>HouseServe</h2>
