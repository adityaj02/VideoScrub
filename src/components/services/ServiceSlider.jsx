                ...
                <div className="w-full max-w-7xl relative h-full">
                    {SERVICES.map((service, idx) => (
                        <div key={service.id} className={`absolute inset-0 flex items-center justify-center transition-all duration-[1200ms] ease-[cubic-bezier(0.19,1,0.22,1)] ${idx === activeIdx ? 'service-card-active' : 'service-card-hidden'}`}>
                            <div className={`glass reflect-card w-full h-[550px] md:h-[600px] rounded-[40px] md:rounded-[64px] overflow-hidden flex flex-col md:flex-row border transition-all duration-700 ${colors.glass} ${isInCart?.(service.id) ? 'in-cart-highlight' : ''}`}>
                                <div className="w-full md:w-[50%] h-56 md:h-full relative overflow-hidden shrink-0">
                                ...
                                <div className="w-full md:w-[50%] flex flex-col h-full bg-transparent p-6 md:p-10 lg:p-14 text-left">
                                    <div className="flex-grow overflow-y-auto custom-scroll pr-2 text-left">
                                        <div className="space-y-6">
                                            ...
                                            <h3 className="text-4xl md:text-6xl lg:text-8xl font-black premium-text tracking-tighter leading-none py-2 overflow-visible text-left">{service.title}</h3>
                                            <p className={`text-sm font-black uppercase tracking-[0.4em] ${colors.subtext}`}>₹{Number(service.price || 0).toLocaleString('en-IN')}</p>
                                            <p className={`text-sm md:text-lg leading-relaxed max-sm font-medium tracking-tight ${colors.cardText}`}>{service.desc}</p>
                                        </div>
                                    </div>
                                    <div className="pt-8">
